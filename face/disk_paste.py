# 由 大龙虾 于 2026-09-18 定稿（前身 wait_paste.py / 面部粘贴稳健，2026-09-17 E2 终版），用于 黑框问题修复：脸从 ComfyUI 预览落盘文件读（40/40 好脸），原图/mask/box 走内存
# 机制定论(9/17): Detailer 输出 tensor 的 CPU 缓冲被后台线程异步覆写，好脸窗口与 0/1 掩码窗口交替，
# 任何时刻直接读内存都是赌。而 PreviewImage 954 存盘的同内容 PNG 40/40 是好脸（harness 每次 run 实测）。
# 本节点接法: Detailer(9) → 面部粘贴（磁盘读取）；trigger 接 954 预览输出，强制其先执行落盘。
#   原图=958, 裁剪图像=9(仅排序锚点), 裁剪数据=4:1, 裁剪遮罩=4:2, trigger=954:0
# [2026-09-18 可移植化] 全部绝对路径移除：ComfyUI 根从本文件位置推导（custom_nodes/<插件>/face/ 的上三级），
#   装在任何位置/任何机器都成立；证据链目录改放 ComfyUI temp 下。
import os
import time
import glob
import cv2
...
# 类注册名：新名 + 旧名别名（9/17-9/18 测试期间用过的旧工作流仍能加载）
NODE_CLASS_MAPPINGS = {
    "面部粘贴（磁盘读取）": 面部粘贴磁盘读取,
    "面部粘贴稳健": 面部粘贴磁盘读取,
}
